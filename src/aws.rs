...
impl<'a> Collector for DefaultCollector<'a> {
    fn match_tag(&self, t: Tag) -> bool {
        ...
        debug!("Checking if {:?} matches {} or {}", t, cluster_id_tag, cluster_name_tag);
        t.key.is_some_and(|t| t.contains(&cluster_id_tag) || t.contains(&cluster_name_tag)) &&
...
pub async fn get_subnets(ec2_client: &EC2Client, subnet_ids: &Vec<String>) -> Result<Vec<Subnet>, aws_sdk_ec2::Error> {
    match ec2_client.describe_subnets().set_subnet_ids(Some(subnet_ids.clone())).send().await {
...
pub async fn get_subnets_by_vpc(ec2_client: &EC2Client, vpc_id: &str) -> Result<Vec<Subnet>, aws_sdk_ec2::Error> {
    debug!("Retrieving subnets for VPC: {}", vpc_id);
    let subnets_filter = Filter::builder().
        name("vpc-id").
        values(vpc_id).
        build();
    match ec2_client.describe_subnets().set_filters(Some(vec![subnets_filter])).send().await {
        Ok(success) => Ok(success.subnets.unwrap()),
        Err(err) => Err(aws_sdk_ec2::Error::from(err)),
    }
}

pub async fn get_route_tables(ec2_client: &EC2Client, subnet_ids: &Vec<String>) -> Result<Vec<RouteTable>, aws_sdk_ec2::Error> {
    debug!("Retrieving route tables for subnets: {}", subnet_ids.join(","));
    let rtb_filter = Filter::builder().
        name("association.subnet-id").
        set_values(Some(subnet_ids.clone())).
        build();
    match ec2_client.describe_route_tables().set_filters(Some(vec![rtb_filter])).send().await {
...
pub async fn get_load_balancers(elb_client: &ELBv2Client, cluster_info: &MinimalClusterInfo) -> Result<Vec<LoadBalancer>, aws_sdk_elasticloadbalancingv2::Error>{
    ...
    let collector: Box<dyn Collector> = match cluster_info.cluster_type {
        ...
        _ => {
            ...
            Box::new(DefaultCollector{cluster_id: &cluster_info.cluster_id, cluster_infra_name: &cluster_info.cluster_infra_name})
        },
    };
